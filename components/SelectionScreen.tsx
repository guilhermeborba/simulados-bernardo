...
type Assessment = 'AV1' | 'AV2' | 'AV3' | 'AV4' | '';
...
export default function SelectionScreen() {
  ...
  const handleYearChange = (year: Year) => {
    ...
    // Se for terceiro ano, mostra o select de avaliação
    if (year === 'terceiro') {
  ...
  const handleAssessmentChange = (assessment: Assessment) => {
    ...
    // Se for AV2, permite continuar. Se for outra, mostra mensagem de indisponibilidade
    if (assessment && assessment !== 'AV2') {
      setShowUnavailableAssessmentMessage(true);
    }
  };

  const getSimuladoPath = () => {
    if (selectedYear === 'terceiro' && selectedAssessment) {
      // Mapeia para a página de simulados com query params
      return `/simulados?year=${selectedYear}&assessment=${selectedAssessment}`;
    }
    return '#';
  };
  ...
  const assessments = [
    ...
    { value: 'AV3', label: 'AV3 - Terceira Avaliação' },
    { value: 'AV4', label: 'AV4 - Quarta Avaliação' },
  ];
            ...
            <div className="animate-in fade-in duration-500">
              ...
              {showUnavailableAssessmentMessage && (
                <div className="mb-6 p-4 bg-amber-50 border-2 border-amber-200 rounded-xl animate-in fade-in duration-300">
                  ...
                  <p className="text-amber-700 mt-2">
                    Desculpe! O simulado da {selectedAssessment} ainda está em desenvolvimento. 
                    No momento, temos apenas o simulado da <strong>AV2</strong> disponível. 
                    Volte em breve! 🚀
                  </p>
                </div>
              )}
              ...
              {selectedAssessment === 'AV2' && (
                <div className="animate-in fade-in duration-500">
                  <Link href={`/simulados?year=${selectedYear}&assessment=${selectedAssessment}`}>
          ...
          {selectedYear && !showUnavailableMessage && (
            <div className="mt-8 p-4 bg-blue-50 border-2 border-blue-200 rounded-xl">
              <p className="text-sm text-blue-700">
                💡 <strong>Dica:</strong> Cada avaliação (AV1, AV2, AV3, AV4) possui um simulado diferente com conteúdos específicos de cada bimestre.
              </p>
            </div>
          )}
        </div>
